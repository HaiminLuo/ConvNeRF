...
from utils import ray_sampling, render_depth, depth_sampling, range_sampling, alpha_sampling
...
class IBRay_NHR(torch.utils.data.Dataset):
    def __init__(self, data_folder_path, transforms, bunch=1024, use_mask=False, num_frame=1, use_depth=False,
                 data_type="NR", no_boundary=False, boundary_width=3, use_alpha=False):
        ...
        if use_depth:
            img, _, _, _, _, _, _ = self.NHR_dataset.__getitem__(0)
            self.render = SingleObjectRender(os.path.join(data_folder_path, "meshes/frame%d.obj" % num_frame),
                                             frame_size=(img.size(2), img.size(1)),
        ...
        if not os.path.exists(os.path.join(data_folder_path, 'mesh_tmp')):
            os.mkdir(os.path.join(data_folder_path, 'mesh_tmp'))
        ...
        if not os.path.exists(os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'rays_0.pt')):
            for i in range(len(self.NHR_dataset)):
                ...
                if self.use_depth:
                    ...
                    depth, far_depth = render_near_far_depth(self.render, opencv_K, cam2world_R, cam2world_t,
                ...
                rays, rgbs, ds = ray_sampling(K.unsqueeze(0), T.unsqueeze(0), (img.size(1), img.size(2)), masks=mask,
                                              mask_threshold=0.5, images=img_rgb.unsqueeze(0), depth=depth, far_depth=far_depth)
                rs = ds[:, 1] - ds[:, 0]
                ds = ds[:, 0]
                als = alpha_sampling(masks=mask, mask_threshold=0.5, alpha_map=alpha_map)
            ...
            torch.save(self.ds, os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'd_0.pt'))

            self.rs = torch.cat(self.rs, dim=0)
            torch.save(self.rs, os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'r_0.pt'))

            self.als = torch.cat(self.als, dim=0)
            torch.save(self.als, os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'a_0.pt'))
            ...
            if self.use_depth:
                for i in range(len(self.NHR_dataset)):
                    img, _, _, T, K, _, _ = self.NHR_dataset.__getitem__(i)
                    self.Ts.append(T.numpy())
                    self.Ks.append(K.numpy())
            ...
            self.ds = torch.load(os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'd_0.pt'))
            self.rs = torch.load(os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'r_0.pt'))
            self.als = torch.load(os.path.join(os.path.join(data_folder_path, 'rays_tmp'), 'a_0.pt'))
...
class IBRay_NHR_View(torch.utils.data.Dataset):
    ...
    def __init__(self, data_folder_path, transforms, use_mask=False, num_frame=1, use_depth=False, data_type="NR",
                 no_boundary=False, b_width=2, use_range=False, use_alpha=False, use_bg=False, synthesis=False):
        ...
        self.use_alpha = use_alpha
        self.NHR_dataset = IBRDynamicDataset(data_folder_path, num_frame, use_mask, transforms,
                                             skip_step=1, random_noisy=0, holes='None', use_depth=use_depth,
                                             use_bg=use_bg, use_alpha=use_alpha, data_type=data_type,
                                             synthesis=synthesis)

        self.data_folder_path = data_folder_path
        self.no_bounadry = no_boundary
    ...
    def __getitem__(self, index, no_random=False):
        ...
        if self.use_depth:
            ...
            if self.no_bounadry:
                ...
                kernel = np.ones((self.b_width, self.no_bounadry), np.uint8)
                depth_mask = cv2.erode(depth_mask, kernel, iterations=1)
        ...
        if self.use_mask:
            ...
            img_rgb[:3, alpha == 0] = img_rgb[3:6, alpha == 0]

        depth = depth.unsqueeze(0)
        alpha = alpha.unsqueeze(0)

        rays, rgbs, ds = ray_sampling(K.unsqueeze(0), T.unsqueeze(0), (img.size(1), img.size(2)),
                                      images=img_rgb.unsqueeze(0), depth=depth, far_depth=far_depth,
        ...
        return rays, \
               rgbs, \
               self.bbox.repeat(rays.size(0), 1, 1), \
               img_rgb, \
               img[3, :, :].unsqueeze(0), \
               img[4, :, :].unsqueeze(0), \
               near_far.repeat(rays.size(0), 1), \
               frame_id, \
               ds, \
               depth, \
               rs, \
