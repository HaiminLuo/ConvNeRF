...
class RaySamplePoint_Near_Far(nn.Module):
    ...
    def forward(self, rays, near_far, test_mode=False):
        '''
        :param rays: N*6
        :param bbox: N*8*3  0,1,2,3 bottom 4,5,6,7 up
        pdf: n*coarse_num 表示权重
        ...
        if test_mode:
            r = 0.2
            mul = torch.rand(near_far[:, 0:1].size(), device=rays.device)
            near_far[:, 0:1] = near_far[:, 0:1] + mul * (near_far[:, 1:2] - near_far[:, 0:1] - r)
            near_far[:, 1:2] = near_far[:, 0:1] + r
...
class RaySamplePoint_Depth(nn.Module):
    def __init__(self, sample_num=32, n_importance=0, sample_inf=False):
        super(RaySamplePoint_Depth, self).__init__()
        self.sample_num = sample_num
        self.sample_inf = sample_inf
        self.n_importance = n_importance

    def forward(self, rays, depth, depth_field=0.1, ratio=0.25, rs=None, test_mode=False):
        '''
        :param rs: N*1
        :param depth: N*1
        :param depth_field: float
        :param ratio: float
        :param rays: N*6
        :return: N*C*3
        '''
        ...
        near = depth - depth_field * ratio
        far = near + depth_field
        if rs is not None:
            df = nn.ReLU()(depth + rs + 0.1 - far)
            far = far + df

        far_importance = near + depth_field

        infi = 1e3
        mask = depth > 0

        t_vals = torch.linspace(0., 1., steps=self.sample_num, device=rays.device)
        z_vals = near.view(-1, 1) * (1. - t_vals).unsqueeze(0).repeat(n, 1) + far.view(-1, 1) * (
            t_vals.unsqueeze(0).repeat(n, 1))

        t_vals = torch.linspace(0., 1., steps=self.n_importance, device=rays.device)
        iz_vals = near.view(-1, 1) * (1. - t_vals).unsqueeze(0).repeat(n, 1) + far_importance.view(-1, 1) * (
            t_vals.unsqueeze(0).repeat(n, 1))

        if self.n_importance > 0:
            z_vals = torch.cat([z_vals, iz_vals], dim=1).sort(dim=1)[0]
        ...
        if self.sample_inf:
            pad = infi * torch.ones_like(z_vals[...,:1], device=z_vals.device)
            z_vals = torch.cat([z_vals, pad], dim=-1)

        # ray_d = ray_d / (ray_d[:,2][...,:,None] + 1e-10)

        pts = ray_o[..., None, :] + ray_d[..., None, :] * z_vals[..., :, None]

        return z_vals.unsqueeze(-1), pts, mask
