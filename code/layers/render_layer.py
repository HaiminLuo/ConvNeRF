...
def gen_weight(sigma, delta, act_fn=F.relu, transimission=False):
    ...
    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1), device=alpha.device), weight], -1), -1)[
                      :,
                      :-1]
    if transimission:
        T = torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1), device=alpha.device), weight], -1), -1)[:, :-1]
        return weights, T
...
class VolumeRenderer(nn.Module):
    def __init__(self, use_mask=False, use_alpha=False, synthesis=False, boarder_weight=1e10, sample_inf=False):
