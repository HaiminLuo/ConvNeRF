...
def render(model, K, T, img_size, ROI=None, bboxes=None, only_coarse=False, near_far=None, depth=None, far_depth=None):
    model.eval()
    assert not (bboxes is None and near_far is None and depth is None), ' either bbox or near_far should not be None.'
    mask = torch.ones(img_size[0], img_size[1])
    if ROI is not None:
        mask = torch.zeros(img_size[0], img_size[1])
        mask[ROI[0]:ROI[0] + ROI[2], ROI[1]:ROI[1] + ROI[3]] = 1.0
    rays, _, ds = ray_sampling(K.unsqueeze(0), T.unsqueeze(0), img_size, masks=mask.unsqueeze(0), depth=depth, far_depth=far_depth)
    rs = ds[:, 1] - ds[:, 0]
    ds = ds[:, 0]
    ...
    with torch.no_grad():
        stage2, stage1, _ = batchify_ray(model, rays, bboxes, near_far=near_far, depth=ds, rs=rs)
    ...
    rgb_features = stage2[3].reshape(img_size[0], img_size[1], -1)
    density_features = stage2[4].reshape(img_size[0], img_size[1], -1)
    ...
    stage2_final[3] = rgb_features
    stage2_final[4] = density_features
